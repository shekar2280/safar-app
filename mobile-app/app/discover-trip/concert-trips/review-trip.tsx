...
export default function ReviewTrip() {
  ...
  const { concertData, setConcertData } = context;
  ...
  const reviewItems = [
    {
      icon: "📍",
      label: "Destination",
      value: concertData?.locationInfo?.name || "TBA",
    },
    {
      icon: (
        <Ionicons
    ...
    },
    {
      icon: "🧑‍🤝‍🧑",
      label: "Traveler",
      value: concertData?.travelers?.title,
    },
    {
      icon: "📅",
      label: "Concert Date",
      value: concertData?.locationInfo?.concertDate 
        ? moment(concertData.locationInfo.concertDate).format("ddd, DD MMM YYYY")
        : "TBA",
    },
    {
      icon: "💰",
  ...
  const onBuildTrip = () => {
    ...
    router.replace("discover-trip/concert-trips/generate-trip" as any);
  };
